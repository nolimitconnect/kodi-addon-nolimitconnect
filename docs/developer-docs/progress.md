...
- Date: 2026-06-06
- Overall phase: Startup configuration workflow
...
- UI migration direction: Kodi settings schema driving startup identity/network config and a run trigger for engine startup
...
| M4 | Core dialogs and settings UX | In Progress | Startup identity/network config now replaces debug and Messenger settings |
...
| P1 | Finalize startup identity + network configuration for engine launch | In Progress | Copilot |
| P1 | Implement Messenger (slot 8) end-to-end MVP with Kodi dialogs and bridge commands/events | In Progress (Python scaffold landed) | Copilot |
...
- [ ] Fresh profile test: `user_name` empty -> add-on emits prompt request event and does not mark network online.
- [ ] Identity submit test: GUI sends `kProvideDisplayName` command -> setting persists and join command is dispatched.
- [ ] Returning user test: non-empty `user_name` at startup -> add-on dispatches join command automatically.
- [ ] Settings edit test: changing `user_name` through settings updates sign-on flow and re-triggers join dispatch.
...
- 2026-06-07: Checkpointed post-UX stabilization state and started native `run_now` trigger hookup so Run section execution can drive sign-on/engine login path in C++ bridge flow.
