...
#if defined(_MSC_VER)
      // Treat the single registers as arrays of floats
      float* s_lanes = (float*)&s_inst_hadd_256;
      float* e_lanes = (float*)&e_128;

      s_acum += s_lanes[0];
      e_lanes[0] = s_acum - y[i];

      s_acum += s_lanes[4];
      e_lanes[1] = s_acum - y[i];

      s_acum += s_lanes[1];
      e_lanes[2] = s_acum - y[i];

      s_acum += s_lanes[5];
      e_lanes[3] = s_acum - y[i];
#else
