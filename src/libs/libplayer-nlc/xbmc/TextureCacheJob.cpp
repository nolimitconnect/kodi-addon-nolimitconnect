...
bool CTextureCacheJob::CacheTexture(std::unique_ptr<CTextureBase>* out_texture)
{
  ...
  std::unique_ptr<CTextureBase> texture = LoadTextureImage(image, width, height, additional_info, true);
  if (texture)
  {
    if (texture->HasAlpha())
...
std::string CTextureCacheJob::DecodeImageURL(const std::string &url, unsigned int &width, unsigned int &height, CPictureScalingAlgorithm::Algorithm& scalingAlgorithm, std::string &additional_info)
{
  ...
  if (StringUtils::StartsWith(url, "image://"))
  {
    ...
    if (!CTextureCache::CanCacheImageURL(thumbURL))
      return "";
    if (thumbURL.GetPwdUserName() == "music" || thumbURL.GetPwdUserName() == "video")
      additional_info = thumbURL.GetPwdUserName();
    if (StringUtils::StartsWith(thumbURL.GetPwdUserName(), "video_") ||
        StringUtils::StartsWith(thumbURL.GetPwdUserName(), "pvr") ||
        StringUtils::StartsWith(thumbURL.GetPwdUserName(), "epg"))
      additional_info = thumbURL.GetPwdUserName();
...
std::unique_ptr<CTextureBase> CTextureCacheJob::LoadTextureImage(const std::string& image,
                                                      unsigned int width,
                                                      unsigned int height,
