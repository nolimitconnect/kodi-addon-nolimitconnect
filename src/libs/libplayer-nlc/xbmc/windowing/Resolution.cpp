  ...
  for (const auto &mode : indexList)
  {
    ...
    const RESOLUTION_INFO info = CServiceBroker::GetWinSystem()->GetGfxContext().GetResInfo(i);
          ...
          MathUtils::FloatEquals(info.fRefreshRate, fps * 2.5f, 0.01f))
      {
        CLog::Log(
            LOGDEBUG,
            "[WHITELIST] Matched an exact resolution with a 3:2 pulldown refresh rate %s (%d)",
            info.strMode.c_str(), i);
        unsigned int pen = abs(info.iScreenHeight - height) + abs(info.iScreenWidth - width);
        if (pen < penalty)
        {
          resolution = i;
          ...
          SETTING_VIDEOSCREEN_WHITELIST_PULLDOWN))
  {
    ...
    for (const auto& mode : indexList)
    {
          ...
          MathUtils::FloatEquals(info.fRefreshRate, fps * 2.5f, 0.01f))
      {
        CLog::Log(
            LOGDEBUG,
            "[WHITELIST] Matched a desktop resolution with a 3:2 pulldown refresh rate %s (%d)",
            info.strMode.c_str(), i);
        resolution = i;
        return;
      }
...
void CResolutionUtils::PrintWhitelist()
{
  ...
  if (!indexList.empty())
  {
    ...
    CLog::Log(LOGDEBUG, "[WHITELIST] whitelisted modes:%s", modeStr.c_str());
  }
}
