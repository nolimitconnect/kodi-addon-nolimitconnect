...
bool CWin32SMBDirectory::ConnectAndAuthenticate(NlcUrl& url, bool allowPromptForCredential /*= false*/)
{
  ...
  if (url.GetPwdUserName().empty() && url.GetPassWord().empty())
  ...
  if (!url.GetPwdUserName().empty() && !g_charsetConverter.utf8ToW(url.GetPwdUserName(), usernameW, false, false, true))
  {
    CLog::LogF(LOGERROR, "Can't convert username \"%s\" to wide string", url.GetPwdUserName().c_str());
    return false;
  }
  std::wstring domainW;
  ...
  if (url.GetPwdUserName().empty())
    loginDescr = "without username";
  else
    loginDescr = "with username \"" + url.GetPwdUserName() + (url.GetDomain().empty() ? "" : "@" + url.GetDomain()) + "\"";
