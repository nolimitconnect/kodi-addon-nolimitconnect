...
bool CWinLibraryFile::IsValid(const NlcUrl & url)
{
...
bool CWinLibraryFile::Delete(const NlcUrl & url)
{
  bool success = false;
...
bool CWinLibraryFile::Rename(const NlcUrl & urlCurrentName, const NlcUrl & urlNewName)
{
  ...
  if (currFile)
  {
    ...
    // move
    NlcUrl defFolder = NlcUrl(urlNewName.GetWithoutFilename());
...
bool CWinLibraryFile::OpenIntenal(const NlcUrl& url, FileAccessMode mode)
{
  ...
  try
  {
    ...
    { 
      auto destFolder = NlcUrl(URIUtils::GetParentPath(url.Get()));
