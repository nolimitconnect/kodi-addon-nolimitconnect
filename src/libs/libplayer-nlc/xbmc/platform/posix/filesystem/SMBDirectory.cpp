        ...
        if ((m_flags & DIR_FLAG_NO_FILE_INFO)==0 && CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_sambastatfiles)
        {
          ...
          if( smbc_stat(strFullName.c_str(), &info) == 0 )
          {
            ...
            if (smbc_getxattr(strFullName.c_str(), "system.dos_attr.mode", value, sizeof(value)) > 0)
            {
              ...
              CLog::Log(LOGERROR, "Getting extended attributes for the share: '%s'\nunix_err:'%x' error: '%s'", NlcUrl::GetRedacted(strFullName).c_str(), errno, strerror(errno));
            ...
            CLog::Log(LOGERROR, "%s - Failed to stat file %s", __FUNCTION__, NlcUrl::GetRedacted(strFullName).c_str());
...
int CSMBDirectory::OpenDir(const NlcUrl& url, std::string& strAuth)
{
  ...
  CLog::LogF(LOGDEBUG, LOGSAMBA, "Using authentication url %s", NlcUrl::GetRedacted(s).c_str());
  ...
  if (fd < 0)
  {
    ...
    CLog::Log(LOGERROR, "SMBDirectory->GetDirectory: Unable to open directory : '%s'\nunix_err:'%x' error : '%s'", NlcUrl::GetRedacted(strAuth).c_str(), errno, strerror(errno));
  }
