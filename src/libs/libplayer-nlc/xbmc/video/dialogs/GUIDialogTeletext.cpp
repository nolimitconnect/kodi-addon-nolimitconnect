...
void CGUIDialogTeletext::Render()
{
  ...
  UTILS::COLOR::Color color = (static_cast<UTILS::COLOR::Color>(teletextFadeAmount * 2.55f) & 0xff) << 24 | 0xFFFFFF;
  CGUITextureBase::DrawQuad(m_vertCoords, color, m_pTxtTexture.get());
...
void CGUIDialogTeletext::OnInitWindow()
{
  ...
  m_pTxtTexture =
      CTextureBase::CreateTexture(m_TextDecoder.GetWidth(), m_TextDecoder.GetHeight(), XB_FMT_A8R8G8B8);
  if (!m_pTxtTexture)
  {
