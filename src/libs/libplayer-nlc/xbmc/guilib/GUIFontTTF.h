...
#include <libfreetype/harfbuzz/src/hb.h>
...
public:
  ...
  static CGUIFontTTFBase* CreateGUIFontTTF(const std::string& fontIdent);
  ...
  const std::string& GetFontIdent() const { return m_fontIdent; }
    ...
    unsigned int m_nTexture{0}; // needs to be public
    std::unique_ptr<CTextureBase> m_texture; // texture that holds our rendered characters (8bit alpha only)


  //explicit CGUIFontTTFBase(const std::string& fontIdent);
  ...
  virtual std::unique_ptr<CTextureBase> ReallocTexture(unsigned int& newHeight) = 0;
