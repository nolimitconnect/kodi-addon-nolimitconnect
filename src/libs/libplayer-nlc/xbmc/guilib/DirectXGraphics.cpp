...
void GetTextureFromData(D3DTexture* pTex, void* texData, std::unique_ptr<CTextureBase>* ppTexture)
{
  XB_D3DFORMAT fmt;
  DWORD width, height, pitch, offset;
  ParseTextureHeader(pTex, fmt, width, height, pitch, offset);

  *ppTexture = CTextureBase::CreateTexture(width, height, XB_FMT_A8R8G8B8);
...
/* DXMERGE - We should really support DXT1,DXT2 and DXT4 in both renderers
             Perhaps we should extend CTextureBase::Update() to support a bunch of different texture types
