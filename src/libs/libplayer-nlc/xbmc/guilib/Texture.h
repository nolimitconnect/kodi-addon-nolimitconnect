...
public:
  CTextureBase(unsigned int width = 0, unsigned int height = 0, XB_FMT format = XB_FMT_A8R8G8B8);
  virtual ~CTextureBase();

  static std::unique_ptr<CTextureBase> CreateTexture(unsigned int width = 0,
  ...
  /*! \brief Load a texture from a file
   ...
   \return a CTextureBase std::unique_ptr to the created texture - nullptr if the texture failed to load.
   */
  static std::unique_ptr<CTextureBase> LoadFromFile(const std::string& texturePath,
  ...
  /*! \brief Load a texture from a file in memory
   ...
   \return a CTextureBase std::unique_ptr to the created texture - nullptr if the texture failed to load.
   */
  static std::unique_ptr<CTextureBase> LoadFromFileInMemory(unsigned char* buffer,
                                                        size_t bufferSize,
...
private:
  // no copy constructor
  //CTextureBase(const CTextureBase& copy);
};
