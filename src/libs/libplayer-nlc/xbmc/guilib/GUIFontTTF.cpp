...
#include <libfreetype/harfbuzz/src/hb-ft.h>
...
CGUIFontTTFBase::CGUIFontTTFBase(const std::string& fontIdent)
    ...
    const std::string& strFilename, float height, float aspect, float lineSpacing, bool border)
{
  ...
  m_textureWidth = CTextureBase::PadPow2(m_textureWidth);
...
void CGUIFontTTFBase::DrawTextInternal(CGraphicContext& context,
                                   float x,
                                   float y,
...
float CGUIFontTTFBase::GetTextWidthInternal(const vecText& text)
{
  const std::vector<Glyph> glyphs = GetHarfBuzzShapedGlyphs(text);
...
float CGUIFontTTFBase::GetTextWidthInternal(const vecText& text, const std::vector<Glyph>& glyphs)
{
  float width = 0;
...
unsigned int CGUIFontTTFBase::GetMaxFontHeight() const
{
  return m_maxFontHeight + SPACING_BETWEEN_CHARACTERS_IN_TEXTURE;
}

std::vector<CGUIFontTTFBase::Glyph> CGUIFontTTFBase::GetHarfBuzzShapedGlyphs(const vecText& text)
{
...
CGUIFontTTFBase::Character* CGUIFontTTFBase::GetCharacter(character_t chr, FT_UInt glyphIndex)
{
...
bool CGUIFontTTFBase::CacheCharacter(FT_UInt glyphIndex, uint32_t style, Character* ch)
{
  ...
  if (!isEmptyGlyph)
  {
    ...
    { // no space - gotta drop to the next line (which means creating a new texture and copying it across)
      ...
      if (m_posY + GetTextureLineHeight() >= m_textureHeight)
      {
        ...
        std::unique_ptr<CTextureBase> newTexture = ReallocTexture(newHeight);
        if (!newTexture)
        {
          FT_Done_Glyph(glyph);
...
void CGUIFontTTFBase::RenderCharacter(CGraphicContext& context,
                                  float posX,
                                  float posY,
                                  const Character* ch,
...
float CGUIFontTTFBase::GetTabSpaceLength()
{
