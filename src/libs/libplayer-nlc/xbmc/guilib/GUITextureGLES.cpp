...
{
  CGUITextureBase::Register(CGUITextureGLES::CreateTexture, CGUITextureGLES::DrawQuad);
}

CGUITextureBase* CGUITextureGLES::CreateTexture(
...
{
  CTextureBase* texture = m_texture.m_textures[m_currentFrame].get();
  texture->LoadToGPU();
  if (m_diffuse.size())
