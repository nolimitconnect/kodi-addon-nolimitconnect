...
public:
  ...
  struct Animation
  {
    std::vector<std::pair<std::unique_ptr<CTextureBase>, int>> textures;
    int width;
    int height;
    int loops;
  };
...
private:
  bool OpenBundle();
  std::unique_ptr<CTextureBase> ConvertFrameToTexture(const std::string& name, const CXBTFFrame& frame);
