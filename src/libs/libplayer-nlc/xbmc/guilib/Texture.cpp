...
/*                                                                      */
/************************************************************************/
CTextureBase::CTextureBase(unsigned int width, unsigned int height, XB_FMT format)
{
  m_pixels = NULL;
  m_loadedToGPU = false;
...
{
  VxAlignedFree( m_pixels, true );
  m_pixels = nullptr;
}

void CTextureBase::Allocate(unsigned int width, unsigned int height, XB_FMT format)
{
...
void CTextureBase::Update(unsigned int width,
                      unsigned int height,
                      unsigned int pitch,
                      XB_FMT format,
...
std::unique_ptr<CTextureBase> CTextureBase::LoadFromFile(const std::string& texturePath,
                                                 ...
                                                 const std::string& strMimeType)
{
  ...
  if (url.IsProtocol("androidapp"))
  {
    ...
    if (file.Open(url))
    {
      ...
      std::unique_ptr<CTextureBase> texture = CTextureBase::CreateTexture();
...
std::unique_ptr<CTextureBase> CTextureBase::LoadFromFileInMemory(unsigned char* buffer,
                                                         size_t bufferSize,
...
bool CTextureBase::LoadFromFileInternal(const std::string& texturePath,
...
bool CTextureBase::LoadFromFileInMem(unsigned char* buffer,
                                 size_t size,
                                 const std::string& mimeType,
                                 unsigned int maxWidth,
                                 unsigned int maxHeight)
...
bool CTextureBase::LoadIImage(IImage* pImage,
...
bool CTextureBase::LoadFromMemory(unsigned int width,
                              unsigned int height,
                              unsigned int pitch,
                              XB_FMT format,
                              bool hasAlpha,
...
bool CTextureBase::LoadPaletted(unsigned int width,
                            unsigned int height,
                            unsigned int pitch,
                            XB_FMT format,
                            const unsigned char* pixels,
...
unsigned int CTextureBase::PadPow2(unsigned int x)
{
  --x;
  x |= x >> 1;
  x |= x >> 2;
...
bool CTextureBase::SwapBlueRed(unsigned char* pixels,
                           unsigned int height,
                           unsigned int pitch,
...
unsigned int CTextureBase::GetPitch(unsigned int width) const
{
  switch (m_format)
  {
  case XB_FMT_DXT1:
...
unsigned int CTextureBase::GetRows(unsigned int height) const
{
  switch (m_format)
  {
  case XB_FMT_DXT1:
...
unsigned int CTextureBase::GetBlockSize() const
{
  switch (m_format)
  {
  case XB_FMT_DXT1:
...
void CTextureBase::CreateTextureObject()
{

}

void CTextureBase::DestroyTextureObject()
{

}

void CTextureBase::LoadToGPU()
{

}

void CTextureBase::BindToUnit( unsigned int unit )
{

}

std::unique_ptr<CTextureBase>  CTextureBase::CreateTexture( unsigned int width,
                                                            unsigned int height,
                                                            XB_FMT format )
{
