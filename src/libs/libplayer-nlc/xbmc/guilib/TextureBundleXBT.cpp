...
std::optional<CTextureBundleXBT::Animation> CTextureBundleXBT::LoadAnim(const std::string& filename)
{
  ...
  for (size_t i = 0; i < nTextures; i++)
  {
    ...
    std::unique_ptr<CTextureBase> texture = ConvertFrameToTexture(filename, frame);
    if (!texture)
      return {};
...
std::unique_ptr<CTextureBase> CTextureBundleXBT::ConvertFrameToTexture(const std::string& name,
                                                    ...
                                                    const CXBTFFrame& frame)
{
  ...
  {
    CLog::Log(LOGERROR,
              "CTextureBundleXBT: failed to decompress frame with %lld unpacked bytes to %lld bytes",
