...
bool CFile::Open( const NlcUrl& file, const unsigned int flags )
{
	...
	try
	{
		...
		*/
		...
		NlcUrl authUrl( url );
		if( CPasswordManager::GetInstance().IsURLSupported( authUrl ) && authUrl.GetPwdUserName().empty() )
		...
		catch( CRedirectException* pRedirectEx )
		{
			...
			if( pRedirectEx && pRedirectEx->m_pNewFileImp )
			{
				...
				if( pNewUrl )
				{
					...
					if( CPasswordManager::GetInstance().IsURLSupported( newAuthUrl ) && newAuthUrl.GetPwdUserName().empty() )
...
bool CFile::Rename( const NlcUrl& file, const NlcUrl& newFile )
{
	try
	{
		...
		if( CPasswordManager::GetInstance().IsURLSupported( authUrlNew ) && authUrlNew.GetPwdUserName().empty() )
