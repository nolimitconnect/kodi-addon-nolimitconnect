...
struct nfs_context *CNfsConnection::getContextFromMap(const std::string &exportname, bool forceCacheHit/* = false*/)
{
  ...
  if(it != m_openContextMap.end())
  {
    ...
    uint64_t now = XbmcThreadsNoChrono::SystemClockMillis();
  ...
  if(HandleDyLoad())
  {
    ...
    if(!m_pNfsContext)
    {
      ...
      else 
      {
        ...
        tmp.lastAccessedTime = XbmcThreadsNoChrono::SystemClockMillis();
    ...
    m_lastAccessedTime = XbmcThreadsNoChrono::SystemClockMillis(); //refresh last access time of m_pNfsContext
  }
  return ret;
}
...
bool CNfsConnection::Connect(const NlcUrl& url, std::string &relativePath)
{
  ...
  if( (ret && (exportPath != m_exportPath  ||
      ...
      (XbmcThreadsNoChrono::SystemClockMillis() - m_lastAccessedTime) > CONTEXT_TIMEOUT )
  {
...
void CNfsConnection::resetKeepAlive(std::string _exportPath, struct nfsfh  *_pFileHandle)
{
  ...
  {
    m_lastAccessedTime = XbmcThreadsNoChrono::SystemClockMillis();
  }
...
int64_t CNFSFile::Read(void *lpBuf, size_t uiBufSize)
{
...
int64_t CNFSFile::Write(const void* lpBuf, size_t uiBufSize)
{
  ...
  int64_t chunkSize = gNfsConnection.GetMaxWriteChunkSize() > 32768 ? 32768 : (size_t)gNfsConnection.GetMaxWriteChunkSize();
