  ...
  try
  {
    ...
    else
    {
      ...
      if (CPasswordManager::GetInstance().IsURLSupported(realURL))
      {
        ...
        if (!realURL.GetPwdUserName().empty())
        {
          ...
          if ( realURL.GetPwdUserName() != authUrl.GetPwdUserName()
...
bool CDirectory::Create(const NlcUrl& url)
{
    ...
    try
    {
        ...
        if (CPasswordManager::GetInstance().IsURLSupported(realURL) && realURL.GetPwdUserName().empty())
