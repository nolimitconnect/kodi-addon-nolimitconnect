...
void CSeekHandler::Configure()
{
  ...
  const std::shared_ptr<CSettings> settings = CServiceBroker::GetSettingsComponent()->GetSettings();
    ...
    m_seekDelays.insert(std::make_pair(SEEK_TYPE_VIDEO, 750));
    //m_seekDelays.insert(std::make_pair(SEEK_TYPE_MUSIC, settings->GetInt(CSettings::SETTING_MUSICPLAYER_SEEKDELAY)));
    m_seekDelays.insert(std::make_pair(SEEK_TYPE_MUSIC, 750));
    ...
    std::vector<CVariant> vidSeekList;
    vidSeekList.emplace_back( -600 );
    vidSeekList.emplace_back( -300 );
    vidSeekList.emplace_back( -180 );
    vidSeekList.emplace_back( -60 );
    vidSeekList.emplace_back( -30 );
    vidSeekList.emplace_back( -10 );
    vidSeekList.emplace_back( 10 );
    vidSeekList.emplace_back( 30 );
    vidSeekList.emplace_back( 60 );
    vidSeekList.emplace_back( 180 );
    ...
    SettingPtr vidSteps = settings->GetSettingsManager()->CreateSetting( "list[integer]", CSettings::SETTING_VIDEOPLAYER_SEEKSTEPS, settings->GetSettingsManager() );
    settings->GetSettingsManager()->AddSettingInternal( vidSteps );
    settings->SetList(  CSettings::SETTING_VIDEOPLAYER_SEEKSTEPS, vidSeekList );

    seekTypeSettingMap.insert( std::make_pair( SEEK_TYPE_MUSIC, CSettings::SETTING_MUSICPLAYER_SEEKSTEPS ) );
    std::vector<CVariant> musicSeekList;

    musicSeekList.emplace_back( -60 );
    musicSeekList.emplace_back( -30 );
    musicSeekList.emplace_back( -10 );
    musicSeekList.emplace_back( 10 );
    musicSeekList.emplace_back( 30 );
    musicSeekList.emplace_back( 60 );

    SettingPtr musicSteps = settings->GetSettingsManager()->CreateSetting( "list[integer]", CSettings::SETTING_MUSICPLAYER_SEEKSTEPS, settings->GetSettingsManager() );
    settings->GetSettingsManager()->AddSettingInternal( musicSteps );
    settings->SetList(  CSettings::SETTING_MUSICPLAYER_SEEKSTEPS, musicSeekList );
