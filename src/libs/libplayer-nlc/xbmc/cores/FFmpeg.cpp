...
std::string FFMpegErrorToString(int err)
{
  ...
  av_strerror(err, (char*)text.data(), AV_ERROR_MAX_STRING_SIZE);
  return text;
}
...
#if !defined(ENABLE_NLC_PLAYER) // ff_avutil_log defined in INlcLog.cpp
...
FFmpegExtraData GetPacketExtradata(const AVPacket* pkt, const AVCodecParameters* codecPar)
{
  ...
  while (ret >= 0)
  {
    ...
    if (retExtraData && retExtraDataSize > 0 && retExtraDataSize < FF_MAX_EXTRADATA_SIZE)
    {
      ...
      {
         LogModule( eLogPlayerNlc, LOG_ERROR, "failed to allocate %d bytes for extradata", retExtraDataSize);
       ...
       LogModule( eLogPlayerNlc, LOG_ERROR, "fetching extradata, extradata_size(%d)", retExtraDataSize);
