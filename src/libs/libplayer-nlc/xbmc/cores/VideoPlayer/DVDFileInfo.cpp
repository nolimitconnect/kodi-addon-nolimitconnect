...
bool CDVDFileInfo::ExtractThumb(const CFileItem& fileItem, CTextureDetails& details, int64_t pos)
{
  ...
  CLog::Log(LOGDEBUG,"%s - measured %u ms to extract thumb from file <%s> in %d packets. ", 
        __FUNCTION__, duration.count(), redactPath.c_str(), packetsTried);
  return bOk;
}

std::unique_ptr<CTextureBase> CDVDFileInfo::ExtractThumbToTexture(const CFileItem& fileItem)
{
  ...
  std::unique_ptr<CTextureBase> result{};
  if (nVideoStream != -1)
  {
    std::unique_ptr<CProcessInfo> pProcessInfo(CProcessInfo::CreateInstance());
    std::vector<AVPixelFormat> pixFmts;
    ...
    if (pVideoCodec)
    {
      ...
      CLog::LogF(LOGDEBUG, "seeking to pos %lld ms (total: %dms) in %s", nSeekTo, nTotalLen,
                 redactPath.c_str());
      ...
      if (pDemuxer->SeekTime(static_cast<double>(nSeekTo), true))
      {
        ...
        {
          {
            ...
            result = CTextureBase::CreateTexture(nWidth, nHeight);
        ...
        {
          LogModule( eLogPlayerNlc, LOG_DEBUG, "decode failed in %s after %d packets.", redactPath.c_str(), packetsTried);
        }
      }
    }
  }
  ...
  auto end = std::chrono::steady_clock::now();
  auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(end - start);
  CLog::LogF(LOGDEBUG, "measured %d ms to extract thumb from file <%s> in %d packets. ",
             duration.count(), redactPath.c_str(), packetsTried);
