...
void CDVDSubtitlesLibass::Configure()
{
	...
	std::string defaultFontDir = VxGetFontDirectory();
	...
	ass_set_fonts_dir( m_library, defaultFontDir.c_str() );

	// Load additional fonts into Libass memory
	CFileItemList items;
	// Get fonts from system directory
	if( XFILE::CDirectory::Exists( defaultFontDir.c_str() ) )
	{
		XFILE::CDirectory::GetDirectory( defaultFontDir.c_str(), items,
...
bool CDVDSubtitlesLibass::DecodeDemuxPkt( const char* data, int size, double start, double duration )
{
	...
	if( !m_track )
	{
		CLog::Log( LOGERROR, "%s - No SSA header found.", __FUNCTION__ );
		return false;
	}
...
bool CDVDSubtitlesLibass::CreateTrack()
{
	...
	if( !m_library )
	{
		CLog::Log( LOGERROR, "%s - Failed to create ASS track, library not initialized.", __FUNCTION__ );
		return false;
	}
	...
	if( m_track == NULL )
	{
		CLog::Log( LOGERROR, "%s - Failed to allocate ASS track.", __FUNCTION__ );
		return false;
	}
...
bool CDVDSubtitlesLibass::CreateStyle()
{
	...
	if( !m_library )
	{
		CLog::Log( LOGERROR, "%s - Failed to create ASS style, library not initialized.", __FUNCTION__ );
		return false;
	}

	if( !m_track )
	{
		CLog::Log( LOGERROR, "%s - Failed to create ASS style, track not initialized.", __FUNCTION__ );
		return false;
	}
...
bool CDVDSubtitlesLibass::CreateTrack( char* buf, size_t size )
{
	...
	if( !m_library )
	{
		CLog::Log( LOGERROR, "%s - No ASS library struct (m_library)", __FUNCTION__ );
		return false;
	}
											 ...
											 int* changes )
{
	...
	{
		CLog::Log( LOGERROR, "%s - ASS renderer/ASS track not initialized.", __FUNCTION__ );
		return nullptr;
	}

	if( updateStyle && !subStyle )
	{
		CLog::Log( LOGERROR, "%s - The subtitle overlay style is not set.", __FUNCTION__ );
		return nullptr;
	}
...
{
	CLog::Log( LOGDEBUG, "%s - Start setting up the LibAss style", __FUNCTION__ );
...
ASS_Event* CDVDSubtitlesLibass::GetEvents()
{
	...
	if( !m_track )
	{
		CLog::Log( LOGERROR, "%s -  Missing ASS structs (m_track)", __FUNCTION__ );
		return NULL;
	}
								   ...
								   subtitleOpts* opts )
{
	...
	{
		CLog::Log( LOGERROR, "%s - Missing ASS structs (m_library or m_track)", __FUNCTION__ );
		return ASS_NO_ID;
	}
	...
	if( eventId >= 0 )
	{
		...
		CLog::Log( LOGERROR, "%s - Cannot allocate a new event", __FUNCTION__ );
	return ASS_NO_ID;
}
...
void CDVDSubtitlesLibass::AppendTextToEvent( int eventId, const char* text )
{
	...
	if( !assEvents )
	{
		CLog::Log( LOGERROR, "%s -  Failed append text to Event ID %d, there are no Events",
...
void CDVDSubtitlesLibass::ChangeEventStopTime( int eventId, double stopTime )
{
	...
	if( !assEvents )
	{
		CLog::Log( LOGERROR, "%s -  Failed change stop time to Event ID %d, there are no Events",
				   __FUNCTION__, eventId );
		return;
	}
