...
bool CVideoPlayerVideo::OpenStream( CDVDStreamInfo hint )
{
    ...
    if( m_messageQueue.IsInited() )
    {
        ...
        SendDvdMessage( std::make_shared<CDVDMsgVideoCodecChange>( hint, std::move( codec ) ), 0 );
    }
    else
    {
...
void CVideoPlayerVideo::CloseStream( bool bWaitForBuffers )
{
    ...
    {
        SendDvdMessage( std::make_shared<CDVDMsg>( CDVDMsg::VIDEO_DRAIN ), 0 );
...
inline void CVideoPlayerVideo::SendDvdMessage( std::shared_ptr<CDVDMsg> pMsg, int priority )
{
...
inline MsgQueueReturnCode CVideoPlayerVideo::GetDvdMessage( std::shared_ptr<CDVDMsg>& pMsg,
...
void CVideoPlayerVideo::Process()
{
    ...
    while( !m_bStop )
    {
        ...
        MsgQueueReturnCode ret = GetDvdMessage( pMsg, timeout, iPriority );
        ...
        if( pMsg->IsType( CDVDMsg::GENERAL_SYNCHRONIZE ) )
        {
            ...
            if( std::static_pointer_cast<CDVDMsgGeneralSynchronize>(pMsg)->Wait( 100ms, SYNCSOURCE_VIDEO ) )
            {
                ...
                SendDvdMessage( pMsg, 1 ); /* push back as prio message, to process other prio messages */
            m_droppingStats.Reset();
        }
...
{
    if( m_messageQueue.IsInited() )
        SendDvdMessage( std::make_shared<CDVDMsgInt>( CDVDMsg::PLAYER_SETSPEED, speed ), 1 );
    else
        m_speed = speed;
}
...
void CVideoPlayerVideo::Flush( bool sync )
{
    ...
    SendDvdMessage( std::make_shared<CDVDMsgBool>( CDVDMsg::GENERAL_FLUSH, sync ), 1 );
    m_bAbortOutput = true;
}
...
int CVideoPlayerVideo::CalcDropRequirement( double pts )
{
    ...
    //LogModule( eLogVideoIo, LOG_DEBUG, "m_renderManager.GetStats lateframes %d iRenderPts %3.3f queued %d discard %d", 
