    ...
    do // read line by line
    {
      if (!strncmp(ptr, "size:", 5))
      {
        ...
        if (width > 0 && height > 0)
        {
          ...
          LogModule( eLogPlayerNlc, LOG_ERROR, "%s - parsed extradata: size: %d x %d", __FUNCTION__, width, height);
        }
      }
      /*
...
OverlayMessage CDVDOverlayCodecFFmpeg::Decode(DemuxPacket* pPacket)
{
  ...
  if (!avpkt)
  {
    CLog::Log(LOGERROR, "CDVDOverlayCodecFFmpeg::%s - av_packet_alloc failed: %s", __FUNCTION__,
              strerror(errno));
    return OverlayMessage::OC_ERROR;
  }
