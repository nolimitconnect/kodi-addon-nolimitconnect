...
static UTILS::COLOR::Color bgcolors[5] = { UTILS::COLOR::BLACK,
  UTILS::COLOR::YELLOW,
  UTILS::COLOR::WHITE,
...
void CRenderer::Render(COverlay* o)
{
            ...
            o = Convert( it->overlay_dvd.get(), it->pts );
    ...
    for( std::vector<COverlay*>::iterator it = render.begin(); it != render.end(); ++it )
    {
        ...
        if( text )
        {
            ...
            UTILS::COLOR::Color bgcolor = bgcolors[settings->GetInt( CSettings::SETTING_SUBTITLES_BGCOLOR )];
            int bgopacity = settings->GetInt( CSettings::SETTING_SUBTITLES_BGOPACITY );
            if( bgopacity > 0 && bgopacity < 100 )
            {
                bgcolor = UTILS::COLOR::ChangeOpacity( bgcolor, bgopacity / 100.0f );
            }
            else if( bgopacity == 0 )
            {
