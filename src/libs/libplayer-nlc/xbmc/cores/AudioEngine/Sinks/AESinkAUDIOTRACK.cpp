...
bool CAESinkAUDIOTRACK::Initialize(AEAudioFormat &format, std::string &device)
{
  ...
  while (!m_at_jni)
  {
    ...
    else
    {
      ...
      {
        CLog::Log(LOGWARNING,
                  "Audiobuffer is already very large %3.3f ms - Reducing to a sensible value",
    ...
    CLog::Log(LOGINFO,
              "Created Audiotrackbuffer with playing time of %3.3f ms min buffer size: {} bytes",
...
void CAESinkAUDIOTRACK::GetDelay(AEDelayStatus& status)
{
  ...
  if (usesAdvancedLogging)
  {
    CLog::Log(LOGINFO, "Delay Current: %3.3f ms", d * 1000);
  }
  status.SetDelay(d);
}
...
unsigned int CAESinkAUDIOTRACK::AddPackets(uint8_t **data, unsigned int frames, unsigned int offset)
{
  ...
  if (!isRawPt)
  {
    ...
    {
      CLog::Log(LOGERROR, "Sink got stuck with %3.3f ms - ask AE for reopening", max_stuck_delay_ms);
    ...
    while (written < size)
    {
      ...
      if (loop_written == 0)
      {
        if (!retried)
        {
          ...
          CLog::Log(LOGDEBUG, "Retried to write onto the sink - slept: %3.3f playing: {}",
