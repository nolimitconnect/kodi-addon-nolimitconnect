...
inline CAEChannelInfo CAESinkALSA::GetChannelLayout(const AEAudioFormat& format, unsigned int channels)
{
  ...
  LogModule( eLogPlayerNlc, LOG_DEBUG, "CAESinkALSA::GetChannelLayout - Requested Layout: %s",
            std::string(format.m_channelLayout).c_str());
  LogModule( eLogPlayerNlc, LOG_DEBUG, "CAESinkALSA::GetChannelLayout - Got Layout: %s (ALSA: %s)",
...
bool CAESinkALSA::InitializeHW(const ALSAConfig &inconfig, ALSAConfig &outconfig)
{
  ...
  */
  ...
  if (periodSize < AE_MIN_PERIODSIZE)
  {
    ...
    LogModule( eLogPlayerNlc, LOG_DEBUG, "Audio Driver reports too low periodSize %d - will use %d fragments",
  ...
  LogModule( eLogPlayerNlc, LOG_DEBUG, "CAESinkALSA::InitializeHW - Setting timeout to %d ms", m_timeout);
...
unsigned int CAESinkALSA::AddPackets(uint8_t **data, unsigned int frames, unsigned int offset)
{
  ...
  while (data_left > 0)
  {
    ...
    if (ret < 0)
    {
      LogModule( eLogPlayerNlc, LOG_ERROR, "CAESinkALSA - snd_pcm_writei(%d) %s - trying to recover", ret,
                snd_strerror(ret).c_str());
...
void CAESinkALSA::HandleError(const char* name, int err)
{
  ...
  {
    case -EPIPE:
       LogModule( eLogPlayerNlc, LOG_ERROR,"CAESinkALSA::HandleError(%s) - underrun", name.c_str());
      if ((err = snd_pcm_prepare(m_pcm)) < 0)
        LogModule( eLogPlayerNlc, LOG_ERROR,"CAESinkALSA::HandleError(%s) - snd_pcm_prepare returned %d (%s)", name.c_str(),
                  err, snd_strerror(err).c_str());
      break;
