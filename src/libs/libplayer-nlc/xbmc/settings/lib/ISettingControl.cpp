...
bool ISettingControl::Deserialize(const TiXmlNode *node, bool update /* = false */)
{
  ...
  if (!SetFormat(format))
  {
    s_logger->error("error reading \"%s\" attribute of <control>", SETTING_XML_ATTR_FORMAT);
    return false;
  }
  ...
  if ((strTmp = elem->Attribute(SETTING_XML_ATTR_DELAYED)) != nullptr)
  {
    ...
    {
      s_logger->error("error reading \"%s\" attribute of <control>", SETTING_XML_ATTR_DELAYED);
      return false;
    }
    else
