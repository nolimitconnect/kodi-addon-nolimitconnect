...
#    define OUT_RS(d, v) (d) = av_clipl_int32((v)>>FILTER_SHIFT)
...
#    define FOFFSET (1<<(FILTER_SHIFT-1))
#    define OUT_RS(d, v) (d) = av_clip_int16((v)>>FILTER_SHIFT)
                                   ...
                                   int n, int update_ctx)
{
    ...
    for (dst_index = 0; dst_index < n; dst_index++) {
        ...
        if (i < c->filter_length)
            val  += src[sample_index + i    ] * (FELEM2)filter[i    ];
#ifdef FELEML
        OUT_RS(dst[dst_index], val + (FELEML)val2);
#else
        OUT_RS(dst[dst_index], val + val2);
#endif
