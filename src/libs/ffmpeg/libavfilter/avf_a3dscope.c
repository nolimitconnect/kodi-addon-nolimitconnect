...
static void projection_matrix(float fov, float a, float near_proj, float far_proj,
                              float matrix[4][4])
{
    float f;
    ...
    matrix[0][0] = f * a;
    matrix[1][1] = f;
    matrix[2][2] = -(far_proj + near_proj) / (far_proj - near_proj);
    matrix[2][3] = -1.f;
    matrix[3][2] = -(near_proj * far_proj) / (far_proj - near_proj);
}
