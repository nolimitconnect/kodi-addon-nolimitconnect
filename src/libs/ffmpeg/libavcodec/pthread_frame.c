...
static attribute_align_arg void *frame_worker_thread(void *arg)
{
    ...
    int idx = p - p->parent->threads;
    char threadName[16];
    snprintf(threadName, sizeof(threadName), "av:%.7s:df%d", avctx->codec->name, idx);
    //LogMsg( LOG_VERBOSE, "libavcodec frame_worker_thread thread %d name %s", VxGetCurrentThreadId(), threadName );

    pthread_mutex_lock(&p->mutex);
    while (1) {
        while (atomic_load(&p->state) == STATE_INPUT_READY && !p->die)
        {
#if DEBUG_CODEC_FRAME
            LogMsg( LOG_VERBOSE, "libavcodec1 frame_worker_thread %d %s get state %d STATE_INPUT_READY waiting start", VxGetCurrentThreadId(), threadName, p->state );
#endif // DEBUG_CODEC_FRAME
            pthread_cond_wait(&p->input_cond, &p->mutex);
#if DEBUG_CODEC_FRAME
            LogMsg( LOG_VERBOSE, "libavcodec1 frame_worker_thread %d %s get state %d STATE_INPUT_READY waiting done", VxGetCurrentThreadId(), threadName, p->state );
#endif // DEBUG_CODEC_FRAME
        }

#if DEBUG_CODEC_FRAME
        LogMsg( LOG_VERBOSE, "libavcodec1 frame_worker_thread %d %s do work state %d", VxGetCurrentThreadId(), threadName, p->state );
#endif // DEBUG_CODEC_FRAME
        ...
        {
#if DEBUG_CODEC_FRAME
            LogMsg( LOG_VERBOSE, "libavcodec2 frame_worker_thread %d %s state %d finish setup", VxGetCurrentThreadId(), threadName, p->state );
#endif // DEBUG_CODEC_FRAME
...
#if DEBUG_CODEC_FRAME
        LogMsg( LOG_VERBOSE, "libavcodec2.5 frame_worker_thread %d %s calling codec->cb.decode get state %d", VxGetCurrentThreadId() threadName, p->state );
#endif // DEBUG_CODEC_FRAME
...
#if DEBUG_CODEC_FRAME
        LogMsg( LOG_VERBOSE, "libavcodec3 frame_worker_thread %d %s codec->cb.decode got_frame %d result %d get state %d", VxGetCurrentThreadId(), 
                threadName, p->got_frame, p->result, p->state );
#endif // DEBUG_CODEC_FRAME

        if ((p->result < 0 || !p->got_frame) && p->frame->buf[0])
        {
#if DEBUG_CODEC_FRAME
            LogMsg( LOG_VERBOSE, "libavcodec4 frame_worker_thread %d %s get state %d release buffer", VxGetCurrentThreadId(), threadName, p->state );
#endif // DEBUG_CODEC_FRAME
        ...
        {
#if DEBUG_CODEC_FRAME
            LogMsg( LOG_VERBOSE, "libavcodec5 frame_worker_thread %d %s get state %d ff_thread_finish_setup", VxGetCurrentThreadId(), threadName, p->state );
#endif // DEBUG_CODEC_FRAME
            ff_thread_finish_setup( avctx );
        }
...
#if DEBUG_CODEC_FRAME
        LogMsg( LOG_VERBOSE, "libavcodec6 frame_worker_thread %d %s get state %d lock progress_mutex", VxGetCurrentThreadId(), threadName, p->state );
#endif // DEBUG_CODEC_FRAME
        pthread_mutex_lock(&p->progress_mutex);

#if DEBUG_CODEC_FRAME
        LogMsg( LOG_VERBOSE, "libavcodec7 frame_worker_thread %d %s set state %d STATE_INPUT_READY", VxGetCurrentThreadId(), threadName, p->state );
#endif // DEBUG_CODEC_FRAME
...
#if DEBUG_CODEC_FRAME
        LogMsg( LOG_VERBOSE, "libavcodec8 frame_worker_thread %d %s get state %d unlock progress_mutex", VxGetCurrentThreadId(), threadName, p->state );
#endif // DEBUG_CODEC_FRAME
        pthread_mutex_unlock(&p->progress_mutex);
    }
    pthread_mutex_unlock(&p->mutex);
#if DEBUG_CODEC_FRAME
    LogMsg( LOG_VERBOSE, "libavcodec frame_worker_thread %d %s thread %d EXITING", VxGetCurrentThreadId(), threadName, VxGetCurrentThreadId() );
#endif // DEBUG_CODEC_FRAME
    return NULL;
}
...
{
    // LogMsg( LOG_VERBOSE, "libavcodec update_context_from_thread %d", VxGetCurrentThreadId() );
...
{
    int err;
    // LogMsg( LOG_VERBOSE, "libavcodec update_context_from_user %d", VxGetCurrentThreadId() );
    dst->flags          = src->flags;
...
FF_ENABLE_DEPRECATION_WARNINGS
    ...
    if( err < 0 )
    {
        LogMsg( LOG_ERROR, "libavcodec update_context_from_user %d ERROR %d", VxGetCurrentThreadId(), err );
        return err;
    }
...
static int submit_packet(PerThreadContext *p, AVCodecContext *user_avctx,
                         AVPacket *avpkt)
{
    // LogMsg( LOG_VERBOSE, "libavcodec submit_packet %d", VxGetCurrentThreadId() );
...
#if DEBUG_CODEC_FRAME
    LogMsg( LOG_VERBOSE, "libavcodec frame_worker_thread %d set state %d STATE_SETTING_UP", VxGetCurrentThreadId(), p->state );
#endif // DEBUG_CODEC_FRAME
...
int ff_thread_decode_frame(AVCodecContext *avctx,
                           ...
                           AVPacket *avpkt)
{
    // LogMsg( LOG_VERBOSE, "libavcodec ff_thread_decode_frame %d", VxGetCurrentThreadId() );
    ...
    do {
        ...
        if (atomic_load(&p->state) != STATE_INPUT_READY) {
            ...
            {
                // LogMsg( LOG_VERBOSE, "libavcodec ff_thread_decode_frame %d wait state %d change to STATE_INPUT_READY", VxGetCurrentThreadId(), p->state );
...
{
    // LogMsg( LOG_VERBOSE, "libavcodec ff_thread_report_progress %d", VxGetCurrentThreadId() );
    PerThreadContext *p;
...
{
    // LogMsg( LOG_VERBOSE, "libavcodec ff_thread_await_progress" );
    PerThreadContext *p;
    atomic_int *progress = f->progress ? (atomic_int*)f->progress->data : NULL;
...
void ff_thread_finish_setup(AVCodecContext *avctx) {
    ...
    // LogMsg( LOG_VERBOSE, "libavcodec ff_thread_finish_setup %d", VxGetCurrentThreadId() );
    ...
    // LogMsg( LOG_VERBOSE, "libavcodec ff_thread_await_progress %d set state %d to STATE_SETUP_FINISHED", VxGetCurrentThreadId(), p->state );
...
{
    int i;
    // LogMsg( LOG_VERBOSE, "libavcodec park_frame_worker_threads %d", VxGetCurrentThreadId() );
    async_unlock(fctx);
    ...
    for (i = 0; i < thread_count; i++) {
        ...
        if (atomic_load(&p->state) != STATE_INPUT_READY) {
            ...
            {
                // LogMsg( LOG_VERBOSE, "libavcodec park_frame_worker_threads %d wait state %d change to STATE_INPUT_READY", VxGetCurrentThreadId(), p->state );
                pthread_cond_wait( &p->output_cond, &p->progress_mutex );
            }
            pthread_mutex_unlock(&p->progress_mutex);
        }
...
{
    // LogMsg( LOG_VERBOSE, "libavcodec ff_frame_thread_free %d", VxGetCurrentThreadId() );
    FrameThreadContext *fctx = avctx->internal->thread_ctx;
    const FFCodec *codec = ffcodec(avctx->codec);
    int i;
    ...
    for (i = 0; i < thread_count; i++) {
        ...
        if (ctx->internal) {
            if (p->thread_init == INITIALIZED) {
                // LogMsg( LOG_VERBOSE, "libavcodec ff_frame_thread_free1 %d state %d", VxGetCurrentThreadId(), p->state );
                pthread_mutex_lock(&p->mutex);
                p->die = 1;
                // LogMsg( LOG_VERBOSE, "libavcodec ff_frame_thread_free2 %d signal p->input_cond state %d", VxGetCurrentThreadId(), p->state );
                pthread_cond_signal(&p->input_cond);
                pthread_mutex_unlock(&p->mutex);
...
{
    // LogMsg( LOG_VERBOSE, "libavcodec init_thread" );
    AVCodecContext *copy;
    int err;

    // LogMsg( LOG_VERBOSE, "libavcodec init_thread %d set state %d STATE_INPUT_READY", VxGetCurrentThreadId(), p->state );
...
{
    // LogMsg( LOG_VERBOSE, "libavcodec ff_frame_thread_init thread %d", VxGetCurrentThreadId() );
...
{
    // LogMsg( LOG_VERBOSE, "libavcodec ff_thread_flush %d", VxGetCurrentThreadId() );
    int i;
    FrameThreadContext *fctx = avctx->internal->thread_ctx;
...
int ff_thread_can_start_frame(AVCodecContext *avctx)
{
    ...
    if ((avctx->active_thread_type&FF_THREAD_FRAME) && atomic_load(&p->state) != STATE_SETTING_UP &&
        ...
        // LogMsg( LOG_VERBOSE, "libavcodec ff_thread_can_start_frame %d false", VxGetCurrentThreadId() );
        return 0;
    }

    // LogMsg( LOG_VERBOSE, "libavcodec ff_thread_can_start_frame %d true", VxGetCurrentThreadId() );
    return 1;
}

static int thread_get_buffer_internal(AVCodecContext *avctx, AVFrame *f, int flags)
{
    // LogMsg( LOG_VERBOSE, "libavcodec thread_get_buffer_internal %d", VxGetCurrentThreadId() );
    PerThreadContext *p;
    int err;
...
{
    // LogMsg( LOG_VERBOSE, "libavcodec ff_thread_get_buffer %d", VxGetCurrentThreadId() );
...
{
    // LogMsg( LOG_VERBOSE, "libavcodec ff_thread_get_ext_buffer %d", VxGetCurrentThreadId() );
    int ret;
...
{
    //LogMsg( LOG_VERBOSE, "libavcodec ff_thread_release_buffer %d", VxGetCurrentThreadId() );
    if (!f)
        return;
...
{
    //LogMsg( LOG_VERBOSE, "libavcodec ff_thread_release_ext_buffer %d", VxGetCurrentThreadId() );
    av_buffer_unref(&f->progress);
